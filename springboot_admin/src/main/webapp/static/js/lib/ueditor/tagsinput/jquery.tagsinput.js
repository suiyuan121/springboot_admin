	...
	$(function($) {
		...
		$.fn.addTag = function(value, options) {
			...
			this.each(function() {
				...
				//                    var overMaxLength = tagslist.length >= options.maxLength ? true : false;
				//                    if (overMaxLength == true){
				//                        alert('超出个数限制');
				//                    }
				//                }
				//				if (value !='' && skipTag != true && overMaxLength != true) {
				...
				if (settings.interactive) {
					...
					$(data.fake_input).bind('keypress', data, function(event) {
						if (event.which == event.data.delimiter.charCodeAt(0) || event.which == 13 || event.which == 32) {
							...
							if ($(event.data.real_input).tagLength() >= settings.maxLength) {
								$(event.data.fake_input).val('');
								return;
							}
