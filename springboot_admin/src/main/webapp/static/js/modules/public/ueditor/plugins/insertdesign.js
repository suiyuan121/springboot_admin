...
define([
    ...
    var _getDate = function(page, poptitle) {
        ...
        var __successHandler = function(data, poptitle, page, pageCount) {
            var dataConetxt = {
                title: poptitle,
                content: tpl("ueditorext/senddesign", data),
                style: {
                    width: "800px",
                    marginLeft: "-400px",
                    marginTop: "-220px"
            ...
            if ($(".J-ueditor-pop").length > 0) {
                $(".J-ueditor-pop .pop-content").empty().append(tpl("ueditorext/senddesign", data));
            } else {
            ...
            $('.J-Choice-Design').click(function(){
                var $obj = $(this).parent(),
                    _id = $(this).data('id'),
                    _no = $(this).data('no'),
                    _src = $obj.find('img').attr("src");

                var html = '<p><img origin="ueditor_upload" class="topic-img J-Ifuwo" width="102" height="102" src="' + _src + '@!240x240" data-type="design" data-no="' + _no + '" /></p>';
        ...
        //数据请求
        $.ajax({
            url: 'http://www.fuwo.com/ifuwo/design/own/',
            dataType: 'json',
            type: 'GET',
            async: true,
            data: parameters,
    ...
    var _init = function() {
        ...
        if (window.UE && UE.registerUI) {
            UE.registerUI('design', function(editor, uiName) {
                ...
                var btn = new UE.ui.Button({
                    ...
                    cssRules: 'background-position:-110px -37px!important;',
                    //点击时执行的命令
                    onclick: function() {
