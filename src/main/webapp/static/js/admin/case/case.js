...
var sendData={};
require([ 'jquery','layuiAll',  'admin/common','ui', 'services', 'adminApi'],function(
           $,layuiAll, comJs,services,adminApi){
        //1开始：page对象开始
        var page = {
                   //1.1开始：初始化函数
                   init: function(){
                   ...
                   operation:function(){
                                                                    ...
                                                                    if(fileIdArg === 'upload-cover') {
                                                                        ...
                                                                        if (((limWidth-200 <=w && w<= limWidth+200) && (limHeight-200 <=h && h<= limHeight + 200)) || ((limWidth*2-200<=w && w<= limWidth*2+ 200) && (limHeight*2-200 <=h && h<= limHeight*2+ 300))) {
                                                                            if($('#'+fileIdArg).parent()[0].tagName.toLowerCase() === 'li'){
                                                                                console.log($('#'+fileIdArg).parents('ul').find('li').length);
                              ...
                              //1.2.1结束
                              //1.2.2开始：上传封面，调用函数
                              uploadImg('upload-cover','coverShowDiv',380,300);
                              //1.2.2结束
                              //1.2.3开始：上传户型图，调用函数
                              uploadImg('upload-house-type','house-show',860,480);
                              //1.2.3结束
                              //1.2.4开始
                   ...
                   //1.2结束
                    ...
                    validate:function(){
                                    ...
                                    $("#form-case-add").validate({
                                                                    ...
                                                                    submitHandler: function() {
                                                                                        ...
                                                                                        var caseNo = $('input[name="no"]').val(),
                                                                                            ...
                                                                                            openStr2=$("input[name='status']:checked").val();
                                                                                        //给可售赋boolean值
                                                                                      var  sellValue=false;
                                                                                      if($("input[name='sell']:checked").val()=="true"){
                                                                                          sellValue=true;
                                                                                      }
                                                                                        ...
                                                                                        //如果是可售，填充价格数
                                                                                        var val=$('input:radio[name="sell"]:checked').val();
                                                                                        if(val=="true"){
                                                                                            designCase["price"]= $('input[name="price"]').val();
                                                                                        }
                                                                                        //发送数据
                                                                                        sendData={
                                    ...
                                    //1.4开始：添加案例页面点击确认，保存案例对象
                                    $('.save-btn').click(function() {
                                                   //判断是否要校验价格必填
                                                    var val=$('input:radio[name="sell"]:checked').val();
                                                    if(val=="false"){//价格不可售，移除价格标签，防止被校验
                                                        $('input[name="price"]').remove();
                                                    }
                                                    //只校验表单内容，不提交表单，表单的无action和提交方式
                                                    $("#form-case-add").submit();
                                    });
                                    //1.4结束
                                    var form = layui.form();
            ...
            if (data.result.code == '10000') {
                ...
                if(showId=='#ulAreaId'){
                    html = '<li class="effect-img-item" data-id="">'+
                        ...
                        '</div>'+
                        '</div>'+
                        '<input  name="effectImgDescription"   class="edit-title edited" type="text" disabled="disabled" value="编辑名称" />'+
                        '<a class="effect-img-confirm closed" href="javascript:;">确定</a>'+
                        '</li>';
                    $(showId).append(html);
                }else {
